...
function toEnglishDigits(c) {
  var adelinne = "۰".charCodeAt(0);
  return c = c.replace(/[۰-۹]/g, function (a) {
    return a.charCodeAt(0) - adelinne;
  }), adelinne = "٠".charCodeAt(0), c = c.replace(/[٠-٩]/g, function (a) {
    return a.charCodeAt(0) - adelinne;
  }), c;
}
...
function load() {
  lstat("success", 'متصل '), isIphone = /ipad|iphone|ipod/i.test(navigator.userAgent.toLowerCase());
...
$(function () {
  ...
  $('#myadmin').click(function () {
    leiara ? (leiara = false, $('.notadmin').hide(), $('.isadmin').show(), $('#myadmin').text('أدوات اخرى')) : (leiara = true, $('.isadmin').hide(), $('.notadmin').show(), $('#myadmin').text('أدوات إداريه'));
  });
});
...
function newsock() {
  ...
  socket = io("", {
    ...
    fixSize(), socket.emit("gh", {
      gh: gh
    }), tiger(), $('.ovr div').attr("class", 'label-info').find('div').text('متصل .. يتم تسجيل الدخول'), lstat('success', 'متصل'), $('#tlogins button').removeAttr('disabled'), myid != null && lk != null && cpend && socket.emit("rc", {
      token: token,
      n: lk
    }), getUrlParameter('enter') != null && cpend == false && ($('#u1').val(hash([new Date().getTime()], 256) + '_زائر'), login(1));
  });
  var e = null,
      seana = setInterval(() => {
  ...
  socket.on('msg', function (a) {
    ...
    clearInterval(seana), lstat('danger', 'غير متصل'), retry();
  }), socket.on('connect_error', function (a) {
    clearInterval(seana), $('.ovr div').attr('class', 'label-danger').find('div').text('فشل الاتصال ..'), console.log('connect_error'), lstat('danger', 'غير متصل'), retry();
  }), socket.on('connect_timeout', function (a) {
    clearInterval(seana), $('.ovr div').attr('class', 'label-danger').find("div").text("فشل الاتصال .."), console.log('connect_timeout'), lstat('danger', 'غير متصل'), retry();
  }), socket.on('error', function (a) {
    clearInterval(seana), $('.ovr div').attr('class', 'label-danger').find('div').text('فشل الاتصال ..'), console.log('error'), lstat('danger', 'غير متصل'), retry();
  });
}
...
function pickedemo(a) {
  ...
  var c = a.attr("title"),
      jaslen = $(a.attr('eid'));
  jaslen.parent().find('.tbox').val($(jaslen).parent().find('.tbox').val() + " ف" + c), jaslen.popover('hide').blur();
}
...
function fmute(a) {
  ...
  if (c.hasClass("stopmic")) {
    ...
    if (c.hasClass('sounds')) {
      ...
      antonino == "إيقاف الصوت" ? (c.text('تشغيل الصوت'), c.parent().parent().parent().find('.ismute').show(), d.paused || d.pause()) : (c.text('إيقاف الصوت'), d.paused && d.play(), c.parent().parent().parent().find('.ismute').hide()), c.parent().parent().parent().find('#showpf').hide();
    } else {
...
function fp() {
  ...
  } else alert('الرجاء المحاولة في وقت لاحق');
}
...
function fq(b, c, d) {
  ...
  jearold.srcObject = d, jearold.muted = c, jearold.autoplay = true, jearold.onpause = function () {
    ...
    $(bryndle).text('إيقاف الصوت'), $(bryndle).text('تشغيل الصوت'), b.removeClass('is_speaking');
  }, jearold.onplay = function () {
    var asja = $(jearold).parent().find('.evant>#showpf i.sounds')[0];
    $(asja).text('تشغيل الصوت'), $(asja).text('إيقاف الصوت'), b.addClass('is_speaking');
...
async function fn(a, b) {
  if (a === myid) return void fs('لايمكنك طلب الاتصال مع نفسك');
  var d = ic.get(a, b);
  d || (d = await fr(a, b));
...
function youtubegonder(b) {
  ...
  e.innerText = 'ارسال الى الحائط', e.style.width = '50%', e.style.margin = '2px 0', e.onclick = function () {
    var ryilee = 'https://www.youtube.com/watch?v=' + b;
    $(".tboxbc").val(ryilee), wellgonder(), jovon.remove();
  };
  var f = document.createElement('button');
  f.style.width = '50%', f.style.margin = '2px 0', f.innerText = 'الغاء', f.onclick = function () {
    jovon.remove();
...
function pmsgboot() {
  send('pmsg', {
    msg: xx = prompt('اكتب اسم الفائز في الجولة'),
    msg: document.getElementById.innerHTML = ' مبـروك فوز ' + xx + ' بـ جولةة بـرب حـائط , و حظ اوفر لـ الجميـع 🌻 '
  });
}
...
function mutednot() {
  ...
  arzoi.mute === true ? (document.getElementById('beep').muted = false, $('#mute').css('display', 'none')) : (document.getElementById("beep").muted = true, $('#mute').css('display', 'none'), $('#mute1').css('display', 'block'), $('#mute1').text(' تشغيل صوت الاشعارات'));
}

function mutednot1() {
  var fable = document.getElementById('beep');
  fable.mute === false ? (document.getElementById('beep').muted = true, $('#mute1').css('display', 'none')) : (document.getElementById('beep').muted = false, $('#mute1').css('display', 'none'), $("#mute").css('display', 'block'), $('#mute').text(' كتم صوت الاشعارات'));
}
...
function ondata(d, e) {
  try {
    switch (d) {
      ...
      case 'typing':
        if (myid) {
          var g = $(".w" + e).css('display'),
              norell = $("#c" + e).length;
          g === 'block' && norell > 0 && $('<div class="typing" style="width: 40%;text-align: center;position: absolute;top: 30px;height: 27px;background-color: white;padding: 0px;right: 30px;"><img style="height: 57px;margin-top: -2px;margin-left: 18px;" src="imgs/icon.gif"><span style="">يكتب الان</span></div>').insertAfter(".w" + e + ' .head');
        }
      ...
      case "ok":
        $('.ovr div').attr('class', 'label-success').find('div').text('متصل ..'), tried = 0, setTimeout(function () {
          $('.ovr').remove();
        }, 1500), cpend = false;
        break;
      ...
      case 'not':
        ...
        var o = $($('#not').html()).first(),
            r = getuser(e.user);
        if (e.topic == 'مراقبة') o.find("#isfil").text('مراقبة');else {
          if (e.topic == 'ممنوعة') o.find('#isfil').text('ممنوعة');else {
            if (e.topic == 'رسالة تلقائية') o.find('#isfil').text('رسالة تلقائية');else {
              if (e.topic == '💔 لم يعجبني') o.find('#isfil').text('💔 لم يعجبني');else {
                if (e.topic == '❤️ اعجبني') o.find('#isfil').text('❤️ اعجبني');else {
                  if (e.topic == '😍 احببته') o.find('#isfil').text("😍 احببته");else {
                    if (e.topic == '😂 اضحكني') o.find('#isfil').text('😂 اضحكني');else e.topic == 'إعجاب' ? o.find('#isfil').text('إعجاب') : o.find('#isfil').text('تنبيه');
                  }
                }
              }
            }
      ...
      case 'wetchprofilenot':
        ...
        var s = $($('#wetchprofilenot').html()).first(),
            r = getuser(e.user);
        e.topic == 'زائر' && s.find('#isfill').text('زائر');
      ...
      case "pm":
        if (myid) {
          ...
          if ((e.msg.includes('gif') || e.msg.includes('jpg') || e.msg.includes('jpeg') || e.msg.includes("tiff") || e.msg.includes('tif') || e.msg.includes('png') || e.msg.includes('webp') || e.msg.includes('bmp') || e.msg.includes('svg')) && e.msg.includes('emo/') == false) $("#c" + e.pm).find('.u-msg').html($('<div><i class=\'fa fa-picture-o\'></i> صورة</div>'));else {
            if (e.msg.includes('wav')) $("#c" + e.pm).find('.u-msg').html($('<div><i class=\'fa fa-microphone  \'></i> تسجيل صوتي</div>'));else {
              if (e.msg.includes('mov') || e.msg.includes('mp4') || e.msg.includes('webm') || e.msg.includes('3gpp')) $("#c" + e.pm).find('.u-msg').html($('<div><i class=\'fa fa-file-audio-o\'></i> مقطع فيديو</div>'));else e.msg.includes('x-wav') || e.msg.includes('acc') || e.msg.includes('m4a') || e.msg.includes('mpeg') || e.msg.includes('mp3') || e.msg.includes('midi') ? $("#c" + e.pm).find('.u-msg').html($('<div><i class=\'fa fa-file-video-o\'></i> مقطع صوت</div>')) : $("#c" + e.pm).find('.u-msg').text(gettext($('<div>' + e.msg + '</div>')));
            }
          }
      ...
      case 'ppmsg':
        if (myid) {
          ...
          e.class = 'ppmsgc';
          var l = MsgAdd("#d2", e);
          l.find('.u-msg').append('<label style="margin-top:2px;color:blue" class="fl nosel fa fa-bullhorn">اعلان للسوابر </label>');
        }
      ...
      case 'pmsg':
        if (myid) {
          e.class = 'pmsggcc';
          var l = MsgAdd('#d2', e);
          l.find('.u-msg').append('<label style="margin-top:2px;color:blue" class="fl nosel fa fa-bullhorn">اعلان </label>');
        }
      ...
      case 'level':
        if (myid) {
          e.class = 'pmsgc';
          var l = MsgAdd('#d2', e);
          l.find('.u-msg').append('<label style="margin-top:2px;color:blue" class="fl nosel fa fa-star">مكافئة</label>');
        }
      ...
      case 'clearR':
        ...
        if (myid) {
          e.class = 'pmsgc';
          var l = MsgAdd('#d2', e);
          l.find(".u-msg").append('<label style="margin-top:2px;color:blue" class="fl nosel fa fa-trash">النظـام   |ㅤ</label>');
        }
      ...
      case 'boooootbrb':
        myid && (e.class = "pmsgc", $(function () {
          $('<div class="uzr fl corner borderg mm" style=" "> <img style="margin-left: 1px;margin-top: 1px;background-image: url(&quot;bootbrb1.gif&quot;);" class="fl fitimg hand u-pic"> <div class="uzr fl" style="padding:0px;width:80%"> <div style="padding:0px;width:100%;" class="fl"> <span style="padding: 1px 11px;margin-top: 1px;color: rgb(199, 103, 48);" class="corner nosel u-topic dots fl hand">Boot | بوت</span> </div> <br> <div style="padding: 0px; width: 100%; color: rgb(60, 0, 255);" class=" u-msg   break  fl"> <div style="padding: 0px;width: 100%;color: rgb(60, 0, 255);text-align: left;display: block;margin-top: -17px;" class=" u-msg   break  fl">  <h1 style="display: inline-block;color: red;"></h1>  إنتباه | تم تشغيل بوت الـ برب من 5 نقـأط سيبدأ بعد 5 ثواني ,  اهلاً بـ الجميع ..</div> </div> </div></div>').prependTo('div#d2bc');
          var c = setInterval(function () {
            $('<div class="uzr fl corner borderg mm" style=" "> <img style="margin-left: 1px;margin-top: 1px;background-image: url(&quot;bootbrb.gif&quot;);" class="fl fitimg hand u-pic"> <div class="uzr fl" style="padding:0px;width:80%"> <div style="padding:0px;width:100%;" class="fl"> <span style="padding: 1px 11px;margin-top: 1px;color: rgb(199, 103, 48);" class="corner nosel u-topic dots fl hand">Boot | بوت</span> </div> <br> <div style="padding: 0px; width: 100%; color: rgb(60, 0, 255);" class=" u-msg   break  fl"> <div style="padding: 0px;width: 100%;color: rgb(60, 0, 255);text-align: left;display: block;margin-top: -17px;" class=" u-msg   break  fl">  <h1 style="display: inline-block;color: red;"></h1>  #1111111111 ..</div> </div> </div></div>').prependTo('div#d2bc'), $('<div class="uzr fl corner borderg mm" style=" "> <img style="margin-left: 1px;margin-top: 1px;background-image: url(&quot;bootbrb.gif&quot;);" class="fl fitimg hand u-pic"> <div class="uzr fl" style="padding:0px;width:80%"> <div style="padding:0px;width:100%;" class="fl"> <span style="padding: 1px 11px;margin-top: 1px;color: rgb(199, 103, 48);" class="corner nosel u-topic dots fl hand">Boot | بوت</span> </div> <br> <div style="padding: 0px; width: 100%; color: rgb(60, 0, 255);" class=" u-msg   break  fl"> <div style="padding: 0px;width: 100%;color: rgb(60, 0, 255);text-align: left;display: block;margin-top: -17px;" class=" u-msg   break  fl">  <h1 style="display: inline-block;color: red;"></h1>  #2222222222 ..</div> </div> </div></div>').prependTo('div#d2bc'), $('<div class="uzr fl corner borderg mm" style=" "> <img style="margin-left: 1px;margin-top: 1px;background-image: url(&quot;bootbrb.gif&quot;);" class="fl fitimg hand u-pic"> <div class="uzr fl" style="padding:0px;width:80%"> <div style="padding:0px;width:100%;" class="fl"> <span style="padding: 1px 11px;margin-top: 1px;color: rgb(199, 103, 48);" class="corner nosel u-topic dots fl hand">Boot | بوت</span> </div> <br> <div style="padding: 0px; width: 100%; color: rgb(60, 0, 255);" class=" u-msg   break  fl"> <div style="padding: 0px;width: 100%;color: rgb(60, 0, 255);text-align: left;display: block;margin-top: -17px;" class=" u-msg   break  fl">  <h1 style="display: inline-block;color: red;"></h1>  #3333333333 ..</div> </div> </div></div>').prependTo('div#d2bc');
          }, 8e3);
          setTimeout(function () {
            clearInterval(c);
          }, 45e3), setTimeout(function () {
            $('<div class="uzr fl corner borderg mm" style=" "> <img style="margin-left: 1px;margin-top: 1px;background-image: url(&quot;bootbrb1.jpg&quot;);" class="fl fitimg hand u-pic"> <div class="uzr fl" style="padding:0px;width:80%"> <div style="padding:0px;width:100%;" class="fl"> <span style="padding: 1px 11px;margin-top: 1px;color: rgb(199, 103, 48);" class="corner nosel u-topic dots fl hand">Boot | بوت</span> </div> <br> <div style="padding: 0px; width: 100%; color: rgb(60, 0, 255);" class=" u-msg   break  fl"> <div style="padding: 0px;width: 100%;color: rgb(60, 0, 255);text-align: left;display: block;margin-top: -17px;" class=" u-msg   break  fl">  <h1 style="display: inline-block;color: red;"></h1> إنتهت الجـولة ..</div> </div> </div></div>').prependTo('div#d2bc');
          }, 46e3);
        }));
        break;
      ...
      case 'boooootbrbx2':
        myid && (e.class = 'pmsgc', $(function () {
          $('<div class="uzr fl corner borderg mm" style=" "> <img style="margin-left: 1px;margin-top: 1px;background-image: url(&quot;bootbrb1.gif&quot;);" class="fl fitimg hand u-pic"> <div class="uzr fl" style="padding:0px;width:80%"> <div style="padding:0px;width:100%;" class="fl"> <span style="padding: 1px 11px;margin-top: 1px;color: rgb(199, 103, 48);" class="corner nosel u-topic dots fl hand">Boot | بوت</span> </div> <br> <div style="padding: 0px; width: 100%; color: rgb(60, 0, 255);" class=" u-msg   break  fl"> <div style="padding: 0px;width: 100%;color: rgb(60, 0, 255);text-align: left;display: block;margin-top: -17px;" class=" u-msg   break  fl">  <h1 style="display: inline-block;color: red;"></h1>  إنتباه | تم تشغيل بوت الـ برب من 10 نقـأط سيبدأ بعد 5 ثواني , اهلاً بـ الجميع ..</div> </div> </div></div>').prependTo('div#d2bc');
          var c = setInterval(function () {
            $('<div class="uzr fl corner borderg mm" style=" "> <img style="margin-left: 1px;margin-top: 1px;background-image: url(&quot;bootbrb.gif&quot;);" class="fl fitimg hand u-pic"> <div class="uzr fl" style="padding:0px;width:80%"> <div style="padding:0px;width:100%;" class="fl"> <span style="padding: 1px 11px;margin-top: 1px;color: rgb(199, 103, 48);" class="corner nosel u-topic dots fl hand">Boot | بوت</span> </div> <br> <div style="padding: 0px; width: 100%; color: rgb(60, 0, 255);" class=" u-msg   break  fl"> <div style="padding: 0px;width: 100%;color: rgb(60, 0, 255);text-align: left;display: block;margin-top: -17px;" class=" u-msg   break  fl">  <h1 style="display: inline-block;color: red;"></h1>  #1111111111 ..</div> </div> </div></div>').prependTo('div#d2bc'), $('<div class="uzr fl corner borderg mm" style=" "> <img style="margin-left: 1px;margin-top: 1px;background-image: url(&quot;bootbrb.gif&quot;);" class="fl fitimg hand u-pic"> <div class="uzr fl" style="padding:0px;width:80%"> <div style="padding:0px;width:100%;" class="fl"> <span style="padding: 1px 11px;margin-top: 1px;color: rgb(199, 103, 48);" class="corner nosel u-topic dots fl hand">Boot | بوت</span> </div> <br> <div style="padding: 0px; width: 100%; color: rgb(60, 0, 255);" class=" u-msg   break  fl"> <div style="padding: 0px;width: 100%;color: rgb(60, 0, 255);text-align: left;display: block;margin-top: -17px;" class=" u-msg   break  fl">  <h1 style="display: inline-block;color: red;"></h1>  #2222222222 ..</div> </div> </div></div>').prependTo("div#d2bc"), $('<div class="uzr fl corner borderg mm" style=" "> <img style="margin-left: 1px;margin-top: 1px;background-image: url(&quot;bootbrb.gif&quot;);" class="fl fitimg hand u-pic"> <div class="uzr fl" style="padding:0px;width:80%"> <div style="padding:0px;width:100%;" class="fl"> <span style="padding: 1px 11px;margin-top: 1px;color: rgb(199, 103, 48);" class="corner nosel u-topic dots fl hand">Boot | بوت</span> </div> <br> <div style="padding: 0px; width: 100%; color: rgb(60, 0, 255);" class=" u-msg   break  fl"> <div style="padding: 0px;width: 100%;color: rgb(60, 0, 255);text-align: left;display: block;margin-top: -17px;" class=" u-msg   break  fl">  <h1 style="display: inline-block;color: red;"></h1>  #3333333333 ..</div> </div> </div></div>').prependTo('div#d2bc');
          }, 5e3);
          setTimeout(function () {
      ...
      case 'newstoryGM':
        if (myid) {
          e.class = 'pmsgc';
          var l = MsgAdd('#d2', e);
          l.find('.u-msg').append('<label style="margin-top:2px;color:blue" class="fl nosel fa fa-history">ستوري</label>');
        }
...
function usearch() {
  $('#usearch').val() != lastus && (lastus = $('#usearch').val(), lastus != "" ? $('#usearch').removeClass("bg") : $('#usearch').addClass("bg"), $('#users .uzr').css('display', ""), $.each($.grep(users, function (a) {
    return a.topic.split("ـ").join("").toLowerCase().indexOf(lastus.split("ـ").join("").toLowerCase()) == -1 && a.idreg.indexOf(lastus) != 0 && a.idreg.indexOf(lastus) != 1;
  }), function (a, b) {
...
function emo(a) {
  for (i = 0; i < 10; i++) {
    ...
    if (cardis != null) {
      ...
      d < emos.length && d > -1 && (a = a.replace(nikhil, '$1<img src="emo/' + emos[d] + '" alt="ف$2" title="ف$2" class="emoi">$3'));
    }
  }
...
function agoo(a) {
  ...
  jaidan < 59 && 'الآن', jaidan = jaidan / 60;
  if (jaidan < 59) return parseInt(jaidan) + "د";
  jaidan = jaidan / 60;
  if (jaidan < 24) return parseInt(jaidan) + "س";
  jaidan = jaidan / 24;
  if (jaidan < 30) return parseInt(jaidan) + "ي";
  return jaidan = jaidan / 30, parseInt(jaidan) + "ش";
}
...
function MsgAdd(f, g) {
  ...
  $.each(valin.find("a.uplink"), function (b, c) {
    ...
    if (mariaelisa.match(/video/i)) {
      var e = $('<button class=\'btn fl fa fa-youtube-play\'>عرض الفيديو</button>');
...
function close(a) {
  if (isclose) return;
  isclose = true, window.onbeforeunload = null, prs(), setTimeout('prs();location.href="/";', a || 3e3), lstat('info', 'يتم إعاده الإتصال');
}
...
function OpenProfile(h) {
  ...
  (antheny.im1 || "").split("/").pop().split(".").length > 2 && k.splice(k.length - 1, 1), (j.find('.star').html(""), j.find('.timetoday').text(), j.find('.wetchprf').text(), istogladmin = true, $('.isadmin').hide(), $('.notadmin').show(), $('#myadmin').text('أدوات إداريه'), j.find('.tigertop').text(antheny.evaluation));
  ...
  if (power.setpower) {
    ...
    j.find('.powerbox .userdays').val(0), j.find('.upower').off().click(function () {
      ...
      $.get("cp.nd?cmd=setpower&token=" + token + '&id=' + antheny.idreg.split("#")[1] + '&power=' + q.val() + '&days=' + jrake, function (a) {
        ...
        oleg.err == true ? alert(oleg.msg) : (alert('تم ترقيه العضو'), send('setpoweron', {
          id: antheny.id,
          power: q.val()
        }));
      });
  ...
  chyeanne != null ? (chyeanne.ops != null && (chyeanne.ops.indexOf(getuser(myid).lid) != -1 || chyeanne.owner == getuser(myid).lid || power.roomowner) && (noelya = true), gajuan = '<div class="fl btn btn-primary dots roomh border" style="padding:0px 5px;max-width:180px;" onclick="rjoin(\'' + chyeanne.id + '\')"><img style="max-width:24px;" src=\'' + chyeanne.pic + "'>" + chyeanne.topic + '</div>', j.find('.u-room').html(gajuan), j.find('.u-room').show()) : j.find('.u-room').hide(), noelya ? j.find('.urkick,.umod').show() : j.find('.urkick,.umod').hide(), ismuted(antheny) ? (j.find('.umute').hide(), j.find('.uunmute').show()) : (j.find('.umute').show(), j.find('.uunmute').hide()), j.find(".ureport").hide(), power.bnrpower != true ? j.find('.ubnr').hide() : j.find('.ubnr').show(), power.history != true ? j.find('.uh').hide() : j.find('.uh').show(), power.kick < 1 ? (j.find('.ukick').hide(), j.find('.udelpic').hide(), j.find('.udelpicx').hide(), j.find('.udelpicxx').hide(), j.find('.udelpicxxx').hide()) : (j.find('.ukick').show(), j.find('.udelpic').show(), j.find('.udelpicx').show(), j.find('.udelpicxx').show(), j.find('.udelpicxxx').show()), !power.ban ? j.find('.uban').hide() : j.find('.uban').show(), power.upgrades < 1 ? j.find('.ugift').hide() : j.find('.ugift').show(), !power.meiut ? j.find('.meiut').hide() : j.find('.meiut').show(), (j.find('.uh').css('background-color', "").off().click(function () {
    ...
    popdiv(g, 'كشف النكات'), $.get('uh?token=' + token + '&u2=' + h, function (d) {
    ...
    $(this).css('background-color', 'indianred').text(!antheny.meiut ? 'الغاء الاسكات' : 'اسكات'), send('action', {
      cmd: 'meiut',
      id: h
    }), j.modal('hide');
  })), antheny.meiut ? $('.meiut').css('background-color', 'indianred').text('الغاء الاسكات') : $('.meiut').css('background-color', "").text("اسكات"), (j.find('.uban').css('background-color', "").off().click(function () {
    $(this).css('background-color', 'indianred'), send("action", {
      cmd: 'ban',
      id: h
    }), j.modal('hide');
  }), j.find('.unot').css('background-color', "").off().click(function () {
    var pagen = prompt('اكتب رسالتك', "");
    ...
    }), jerime.append('<button style=\'padding:5px;margin:4px;\' class=\'btn btn-primary hand borderg corner fa fa-ban\'  onclick=\'gift("' + h + '","");$(this).parent().pop("remove")\'>إزاله الهديه</button>'), j.find('.ugift').popover({
      placment: 'bottom',
    ...
    }), tonua.append('<button style=\'padding:5px;margin:4px;\' class=\'btn btn-primary hand borderg corner fa fa-ban\'  onclick=\'ubnr("' + h + '","");$(this).parent().pop("remove")\'>إزاله البنر</button>'), j.find('.ubnr').popover({
      placment: 'bottom',
...
function ChangePassword() {
  var sudarshan = prompt('الرجاء إدخال كلمة المرور الجديدة');
...
function roomkr() {
  ...
  var c = $('#mkr');
  c.find('.rsave').hide(), c.find('.rdelete').hide(), c.find('.modal-title').text('إنشاء غرفه جديدة'), c.modal({
    backdrop: 'static'
    ...
    if (c.find('.rtopic').val().trim().length <= 0) return alert('لا يمكن ترك اسم الغرفة فارغاً ');
    if (c.find('.rabout').val().trim().length <= 0) return alert('لا يمكن ترك وصف الغرفة فارغاً ');
    if (c.find(".rwelcome").val().trim().length <= 0) return alert('لا يمكن ترك  رسالة الترحيب فارغا');
    if (c.find('.colorrooms').val().trim().length <= 0) return alert('لا يمكن ترك  لون الغرفة فارغا');
    if (c.find('.rmax').val().trim().length <= 0 || c.find('.rmax').val() > 1e3 || c.find('.rmax').val() < 2) return alert('يجب ان يكون عدداعظاء الروم لا يزيد عن 40 او اقل من 2');
    send('room+', {
...
function redit(b) {
  ...
  if (d == null) return;
  var e = $('#mkr');
  e.find('.modal-title').text('إداره الغرفه'), e.find('.rsave').show().off().click(function () {
    if (e.find('.rtopic').val().trim().length <= 0) return alert("لا يمكن ترك اسم الغرفة فارغاً ");
    if (e.find('.rabout').val().trim().length <= 0) return alert('لا يمكن ترك وصف الغرفة فارغاً ');
    if (e.find(".rwelcome").val().trim().length <= 0) return alert('لا يمكن ترك  رسالة الترحيب فارغا');
    if (e.find('.rmax').val().trim().length <= 0 || e.find(".rmax").val() > 1e3 || e.find('.rmax').val() < 2) return alert('يجب ان يكون عدداعظاء الروم لا يزيد عن 40 او اقل من 2');
    send('room^', {
      id: b,
...
function sendpicroom() {
  ...
  $(iyahna).on('change', function () {
    ...
    f.append('photo', $(iyahna).prop('files')[0]), h = $.ajax({
      ...
      error: function (a) {
        $('.picroom').attr('src', ""), alert('فشل إرسال الصوره تأكد ان حجم الصوره مناسب');
      }
    });
  });
}
...
function handleOffer(d, e) {
  ...
  $('.callvideoaccept').on('click', function () {
    ...
    astyn && ($(".statecall").text('متصل'), hl($('.statecall'), 'success'), $('#videoCall').find('.u-pic').css('background-image', 'url(' + removegifpic(astyn.pic + ")")), $('#videoCall').find(".u-topic").text(astyn.topic));
...
function handleAnswer(a) {
  $(".statecall").text('متصل'), hl($('.statecall'), 'success'), yourConn.setRemoteDescription(new RTCSessionDescription(a));
}
...
function handleLeave() {
  $('#callvideonot').hide(), $('.statecall').text('رفض'), hl($('.statecall'), 'danger'), $('.vloumemic').removeClass("fa-volume-off"), $('.vloumemic').addClass('fa-volume-up'), $('.mutemic').removeClass('fa-microphone-slash'), $('.mutemic').addClass('fa-microphone'), setTimeout(() => {
    $('#videoCall').hide();
...
function call(c) {
  ...
  if (c.length > 0) {
    ...
    $('.statecall').text('جاري الإتصال'), hl($('.statecall'), 'warning');
...
function aftarpic() {
  ...
  $(taley).on('change', function () {
    ...
    f.append('photo', $(taley).prop('files')[0]), h = $.ajax({
      ...
      error: function (a) {
        $('.spicx').attr('src', ""), alert('فشل إرسال الصوره تأكد ان حجم الصوره مناسب');
      }
    });
  });
}
...
function aftarpicx() {
  ...
  $(amreen).on('change', function () {
    ...
    f.append('photo', $(amreen).prop("files")[0]), h = $.ajax({
      ...
      error: function (a) {
        $('.spicxx').attr('src', ""), alert("فشل إرسال الصوره تأكد ان حجم الصوره مناسب");
      }
    });
  });
}
...
function aftarpicxx() {
  ...
  $(zamyria).on('change', function () {
    ...
    f.append('photo', $(zamyria).prop('files')[0]), h = $.ajax({
      ...
      error: function (a) {
        $(".spicxxx").attr('src', ""), alert('فشل إرسال الصوره تأكد ان حجم الصوره مناسب');
      }
    });
  });
}
